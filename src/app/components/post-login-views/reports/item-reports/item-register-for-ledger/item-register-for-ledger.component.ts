...
import { IItem, ILedger, ItemRegisterForLedgerReportArgument, ItemRegisterForLedgerReportLine, ItemRegisterForLedgerReportServiceGetReportAsFile, ItemRegisterForLedgerReportServiceService, ItemServiceService, LedgerServiceService, PItemMaster, PLedgerMaster } from 'src/server';

@Component({
  selector: 'app-item-register-for-ledger',
  templateUrl: './item-register-for-ledger.component.html',
  styleUrls: ['./item-register-for-ledger.component.css']
})
export class ItemRegisterForLedgerComponent implements OnInit {
  ...
  detailsLines : ItemRegisterForLedgerReportLine[];
  ...
  constructor(private breakpointObserver: BreakpointObserver, private overlayService : OverlayService,
    ...
    private ledgerService : LedgerServiceService, private itemService: ItemServiceService,
    private componentService : ItemRegisterForLedgerReportServiceService, private downloadService : DownloadService) { }
  ...
  getReport() : void{
    ...
    this.componentService.getReportArg(this.searchArg).subscribe({
      next: (data) => {
        this.detailsLines = data.detailsLine || [];
